...
const RESERVED_ACCOUNT_NAMES: &[&str] = &[
    "administrator", "guest", "defaultaccount", "wdagutilityaccount", "helpassistant",
    "krbtgt", "local", "none", "system",
];
...
pub fn generate_autounattend(t: &WindowsTweaks) -> Result<String> {
    ...
    let mut out = vec![
        ...
        "<!-- Generated by Sirius Flash — https://github.com/sirius-ide/sirius-flash -->".to_string(),
    ...
    if !sp.is_empty() {
        ...
        out.extend(component("Microsoft-Windows-Deployment", run_synchronous(&sp)));
        out.push("  </settings>".into());
    }
    ...
    if !first_logon.is_empty() {
        ...
        for (i, c) in first_logon.iter().enumerate() {
            ...
            shell.push(format!("          <CommandLine>{}</CommandLine>", xml_escape(c)));
...
pub fn detect_iso_kind(iso: &Path) -> Result<IsoKind> {
    ...
    if let Ok(o) = Command::new("blkid").args(["-o", "export"]).arg(iso).output() {
        if o.status.success() {
...
pub fn flash_windows_iso(d: &UsbDevice, iso: &Path, tweaks: Option<&WindowsTweaks>) -> Result<()> {
    ...
    let result = (|| -> Result<()> {
        ...
        println!(
            ...
            img_size / 1024 / 1024,
            if split { "will be split for FAT32" } else { "fits FAT32, copied whole" }
        );
        run("rsync", &rsync_args)?;
        if split {
            run("wimlib-imagex", &["split", &format!("{iso_mnt}/sources/{install_img}"), &format!("{usb_mnt}/sources/install.swm"), "3800"])?;
        }
...
#[cfg(not(target_os = "linux"))]
pub fn flash_windows_iso(_d: &UsbDevice, _iso: &Path, _tweaks: Option<&WindowsTweaks>) -> Result<()> {
...
mod tests {
    ...
    fn windows_marker_matches_wim_esd_and_case() {
        ...
        assert!(lists_windows_image("boot/bootx64.efi\nsources/install.wim\n"));
    }
    #[cfg(target_os = "linux")]
    #[test]
    fn windows_marker_rejects_linux_listing() {
        assert!(!lists_windows_image("casper/vmlinuz\nboot/grub/grub.cfg\nEFI/BOOT/BOOTX64.EFI"));
    ...
    fn reserved_account_names_are_rejected() {
        let t = WindowsTweaks { local_account: Some("Administrator".into()), ..Default::default() };
        assert!(generate_autounattend(&t).is_err());
    }

    #[test]
    fn account_names_are_stripped_of_forbidden_characters() {
        let t = WindowsTweaks { local_account: Some("Te:st|User".into()), ..Default::default() };
        assert!(generate_autounattend(&t).unwrap().contains("<Name>TestUser</Name>"));
    }

    #[test]
    fn xml_special_characters_are_escaped() {
        let t = WindowsTweaks { local_account: Some("A&B".into()), ..Default::default() };
        let xml = generate_autounattend(&t).unwrap();
    ...
    fn scratch_sources(tag: &str, files: &[(&str, usize)]) -> PathBuf {
        let dir = std::env::temp_dir().join(format!("sirius-flash-test-{tag}")).join("sources");
